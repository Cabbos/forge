...
- Acceptance: browser smoke coverage for resume, diagnostics, provider probes/model catalog refresh, static fallback catalogs, selection/default saving, compact provider metadata rendering, provider evidence start readiness, custom provider profile templates/add/edit/delete, permissions including the Composer Full Access control, scheduler, A2A review, background task UI, current health alerts, stale-banner scoping, and preview ownership details, plus runtime ownership gates for mocked restart evidence, provider usage, legacy transcript usage hydration, post-shell file effects, persisted A2A lineage, review-to-commit eligibility, gated headless policy/approval checks, and the real Rust `run_worktree_worker` harness.
