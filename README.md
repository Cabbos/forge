...
Eval-runner now also includes file effects evidence scoring for changed-file duplicates, trace/evidence alignment, and file diff completeness when ForgeRunEvidence file effects are present, plus tool/shell evidence scoring for replay identity, command/tool facts, exit-code consistency, trace alignment, and secret-like output leakage.
