...
- Phase 8 desktop UI evidence helpers now surface recovery commands from preflight through the disposable loop status/runbook summaries when local screenshot or window observation is blocked, expose shared `liveReadyGate.pass/reason` diagnostics for the hard gate in status/runbook output, the acceptance matrix runs `--require-live-ready` as a hard gate that requires a checked UI preflight, and archived rows require validation/evidence/markdown sidecars before they are treated as complete.
