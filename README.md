...
- Phase 8 desktop UI evidence helpers now surface recovery commands and `permissionScope` from preflight through the disposable loop status/runbook summaries, including `not_checked` summaries when UI preflight is skipped, make clear that Forge Trust/Full Access does not grant macOS Screen Recording or Accessibility, have the doctor commands, blocker actions, and next step point back to the `--require-live-ready` hard gate after local permission recovery, expose shared `liveReadyGate.pass/reason` diagnostics for the hard gate in status/runbook output, the acceptance matrix runs `--require-live-ready` as a hard gate that requires a checked UI preflight, and archived rows require validation/evidence/markdown sidecars before they are treated as complete.
