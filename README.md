...
- Settings: models with config-defined provider profiles surfaced in the desktop catalog, compact provider metadata rendering, custom provider profile templates and add/edit/delete, no-auth local provider support, provider-aware start readiness, manual provider compatibility probes, live/static model catalog refresh with persisted source labeling and model selection/default saving, workspace, tools, memory, data, diagnostics, scheduler, and general service/autostart surfaces.
