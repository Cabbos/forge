...
- Phase 8 desktop UI evidence helpers now surface recovery commands from preflight through the disposable loop status/runbook summaries when local screenshot or window observation is blocked, the status helper can be run with `--require-live-ready` as a hard automation gate, and archived rows require validation/evidence/markdown sidecars before they are treated as complete.
