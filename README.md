...
The current acceptance suite advertises and runs those runtime gates before the desktop smoke tests, so product claims about durable loop state are backed by replay, policy, gate, completion, review-to-commit eligibility, gateway status, subagent projection, mocked desktop restart evidence as partial macOS proof, an explicit desktop restart harness preflight plus contract and blocker-documentation gates that report the current macOS official WKWebView WebDriver support gap, provider usage known/unknown telemetry, transcript usage hydration, state consistency map status, bounded post-shell file-effect evidence, persisted A2A lineage, gated headless ownership policy/approval checks, A2A child runtime file-IO facts, direct ToolExecutor file-IO stream smoke coverage, and a real Rust `run_worktree_worker` harness using the mock adapter/harness.
