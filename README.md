...
- Acceptance: browser smoke coverage for resume, diagnostics, provider probes/model catalog refresh, static fallback catalogs, selection/default saving, compact provider metadata rendering, provider evidence start readiness, custom provider profile templates/add/edit/delete, permissions including the Composer Full Access control, scheduler, A2A review, background task UI, current health alerts, stale-banner scoping, and preview ownership details, plus an acceptance matrix contract gate, runtime ownership gates for mocked restart evidence, provider usage, transcript usage hydration, state consistency map status, post-shell file effects, persisted A2A lineage, review-to-commit eligibility, gated headless policy/approval checks, and the real Rust `run_worktree_worker` harness.
...
The current acceptance suite advertises and runs those runtime gates before the desktop smoke tests, so product claims about durable loop state are backed by replay, policy, gate, completion, review-to-commit eligibility, gateway status, subagent projection, mocked desktop restart evidence as partial macOS proof, an explicit desktop restart harness preflight plus contract and blocker-documentation gates that report the current macOS official WKWebView WebDriver support gap and require a desktop restart harness launch command before non-macOS can claim official readiness, provider usage known/unknown telemetry, transcript usage hydration, state consistency map status, bounded post-shell file-effect evidence, persisted A2A lineage, gated headless ownership policy/approval checks, A2A child runtime file-IO facts, direct ToolExecutor file-IO stream smoke coverage, and a real Rust `run_worktree_worker` harness using the mock adapter/harness.
