...
- Settings: models with config-defined provider profiles surfaced in the desktop catalog, custom provider profile add/delete, no-auth local provider support, manual provider compatibility probes and model catalog refresh, workspace, tools, memory, data, diagnostics, scheduler, and general service/autostart surfaces.
...
- Acceptance: browser smoke coverage for resume, diagnostics, provider probes/model catalog refresh, custom provider profile add/delete, permissions, scheduler, A2A review, and background task UI, plus runtime ownership gates for mocked restart evidence, provider usage, post-shell file effects, persisted A2A lineage, review-to-commit eligibility, gated headless policy/approval checks, and the real Rust `run_worktree_worker` harness.
