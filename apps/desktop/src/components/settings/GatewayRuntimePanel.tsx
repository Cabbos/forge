...
import { DiagnosticsError } from "./DiagnosticsReportSections";
import { STATUS_CLASS, STATUS_ICON } from "./diagnosticsStatus";
import {
  formatDuration,
  formatGatewayDegradedMode,
  formatRuntimeTaskName,
} from "./diagnosticsFormatters";
import { formatMutationError } from "./settingsUtils";

/** Gateway runtime status, session registry, run history, trigger queue, and enqueue form. */

export function GatewayRuntimePanel({
  status,
  triggers,
  sessions,
  isLoading,
  ...
  const ownershipText = formatGatewayOwnershipSummary(runtime.ownership);
