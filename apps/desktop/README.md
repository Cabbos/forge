...
Phase 8 desktop UI evidence helpers 会在本机截图或窗口观测不可用时，把 recovery commands 从 preflight 透传到 disposable loop status/runbook 摘要，直接指向 doctor 和可选的系统设置打开路径；status/runbook JSON/Markdown 现在共用 `liveReadyGate.pass/reason`，用于解释 hard gate 为什么通过或被阻塞；acceptance matrix 现在运行 `--require-live-ready` 作为自动化硬门禁，这个门禁要求实际跑过 UI preflight，并且只有 validation/evidence/markdown 三件套都存在时才把 row 视为 archived complete。
