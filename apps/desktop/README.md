...
桌面后端已经具备 reference-only 的系统凭据存储层：macOS 生产构建使用 Keychain，不支持系统存储的平台会 fail closed。普通 `app.log` 与结构化 JSONL 日志会在创建、追加或轮转文件之前统一脱敏已登记凭据、鉴权 header、敏感 JSON 字段和 URL query/fragment；脱敏失败会直接抑制该条持久化。OpenAI-compatible 适配器只记录 provider、消息/工具数量和请求字节数，不记录请求正文。现有明文 Settings/profile 的原子迁移仍由后续迁移门负责，不能仅凭这层抽象声称历史配置已经迁移。
