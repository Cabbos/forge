...
Phase 8 desktop UI evidence helpers 会把 recovery commands 和 `permissionScope` 从 preflight 透传到 disposable loop status/runbook 摘要，即使 UI preflight 被跳过、状态是 `not_checked` 也会保留 macOS privacy 边界说明；preflight/doctor 会明确说明 Forge 的 Trust/Full Access 不会授予 macOS Screen Recording 或 Accessibility，并且 doctor 的命令、阻塞项 action、next step 都会在本机权限修复后指回 `--require-live-ready` 硬门禁；status/runbook JSON/Markdown 现在共用 `liveReadyGate.pass/reason`，用于解释 hard gate 为什么通过或被阻塞；acceptance matrix 现在运行 `--require-live-ready` 作为自动化硬门禁，这个门禁要求实际跑过 UI preflight，并且只有 validation/evidence/markdown 三件套都存在时才把 row 视为 archived complete。
