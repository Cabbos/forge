...
- **会话恢复由 durable mutation journal 提供权威，快照只是恢复缓存。** 具体约束：
  1. 会话快照是恢复缓存，不是独立历史；
  2. mutation journal 在快照缺失、损坏或落后时提供恢复；
  3. 内部 journal 事件损坏时 fail closed：journal 被隔离并重新生成，不会静默跳过损坏行；
  4. 没有任何 provider 流或工具进程能在应用重启后幸存。
