...
Presentation logic is extracted into pure, tested modules beside components (e.g. `processToolPresentation.ts`, `writePreviewPresentation.ts`, `diagnosticsFormatters.ts`) — keep components thin and put new derivation logic in those modules.
