...
Live row status: `node scripts/phase8-disposable-loop-status.mjs --json` currently reports `ui_evidence_not_ready` with top-level recovery commands; the next incomplete row is still row #1, but local screenshot/window evidence must be fixed or replaced with a trusted manual desktop session before strict archive.
