...
Status: Archived complete for rows #1/#2/#3.
...
Status: Partially automated; disposable rows #1/#2/#3 archived complete.

Automation and archived row evidence:
...
- Rows #1/#2/#3 evidence validator added on 2026-06-27 and corrected on 2026-06-28: before manual fields were filled it reported pending live evidence; after row-specific manual JSON and build evidence were supplied, strict validation accepted `--manual-json <file>` and produced the archived `2026-06-28-row-{1,2,3}.validation.json` files.
...
- Rows #1/#2/#3 live row runbook added on 2026-06-28 and connected to desktop UI evidence preflight/doctor on 2026-06-28: `node scripts/phase8-disposable-loop-runbook.mjs --json --row 1` now starts with `node scripts/desktop-ui-evidence-preflight.mjs --json --require-ready`, includes `node scripts/desktop-ui-evidence-doctor.mjs --markdown`, reports the UI preflight status, exposes normalized top-level `recoveryCommands` when blocked or `not_checked`, carries shared `liveReadyGate.pass/reason` diagnostics, preserves `permissionScope` and nested recovery commands even for `not_checked` preflight summaries, and separates clean-project readiness from local screenshot/window evidence readiness.
- Rows #1/#2/#3 live row status helper added on 2026-06-28 and connected to desktop UI evidence preflight/recovery on 2026-06-28. It now reports `status: complete`, `nextRow: null`, and `liveReadyGate.reason: "complete"` when all three row archive triplets are present and their validation JSON says `status: "complete"`. It only treats an archive as complete when validation JSON, evidence JSON, and markdown sidecars all exist. `scripts/acceptance.sh` also runs `node scripts/desktop-ui-evidence-doctor.mjs --json --run-checks`, `node scripts/phase8-disposable-loop-status.mjs --json --require-live-ready`, and the beta-log archive-status gate so full acceptance fails if row archives disappear or this log drifts back to a stale `Not yet run` state.
- Rows #1/#2/#3 strict archives completed on 2026-06-28:
  - Row #1: `apps/desktop/docs/product/evidence/phase8-disposable-loop/2026-06-28-row-1.validation.json`, `.evidence.json`, and `.md` record the `/fix @src/App.tsx` visible-feedback run, changed files `src/WaterTracker.tsx` and `src/styles.css`, `npm run build` success, final answer, no confirmation cards, and transcript/screenshot references.
  - Row #2: `apps/desktop/docs/product/evidence/phase8-disposable-loop/2026-06-28-row-2.validation.json`, `.evidence.json`, and `.md` record the CSS-only polish run, changed file `src/styles.css`, `npm run build` success, final answer, no confirmation cards, and transcript/screenshot references.
  - Row #3: `apps/desktop/docs/product/evidence/phase8-disposable-loop/2026-06-28-row-3.validation.json`, `.evidence.json`, and `.md` record the command-only build/check run, no changed files, `npm run build` success, final answer, no confirmation cards, and transcript/screenshot references.
- Rows #1/#2/#3 desktop UI observer preflight added on 2026-06-28 and tightened for screenshot capture/recovery on 2026-06-28: `node scripts/desktop-ui-evidence-preflight.mjs --json` distinguishes Forge runtime evidence from local automation visibility and now includes `recoveryCommands` pointing to the desktop UI evidence doctor, opt-in settings opener, `node scripts/desktop-ui-evidence-doctor.mjs --json --run-checks`, strict preflight rerun, and live-ready hard gate, plus `permissionScope` explaining that Forge Trust/Full Access does not grant macOS Screen Recording or Accessibility. Current local investigation found Tauri dev starts, Vite serves `http://localhost:1420/`, and Forge logs session creation, while System Events reported zero windows for visible apps and macOS screen capture produced a likely blank image; this makes screenshots/window counts untrustworthy for live evidence in this desktop session.
- Rows #1/#2/#3 desktop UI evidence doctor added on 2026-06-28, given an opt-in settings opener on 2026-06-28, and given `--run-checks` on 2026-06-28: `node scripts/desktop-ui-evidence-doctor.mjs --json` maps the current `screen_capture_limited` plus zero-window evidence to Screen Recording and Accessibility recovery commands, records that Forge Trust/Full Access does not grant those macOS permissions, `--open-settings` opens the relevant macOS panes when explicitly requested, `--run-checks` reruns the strict preflight and live-ready hard gate in one command with compact per-check summaries, and the commands, blocker actions, and next step point back to `node scripts/desktop-ui-evidence-preflight.mjs --json --require-ready` plus `node scripts/phase8-disposable-loop-status.mjs --json --require-live-ready`.
...
- Row #9 desktop harness preflight passed on 2026-06-27 as an honest block and was clarified on 2026-06-28: `node scripts/desktop-restart-harness-preflight.mjs --json` reports `blocked_official_macos`, `canRunOfficialHarness: false`, missing `tauri-driver`, a WebDriver client package, and `official macOS WKWebView WebDriver support`, while `node --test scripts/desktop-restart-harness-preflight.test.mjs` and the `desktop restart harness blocker documentation status` acceptance gate now keep this beta log plus the protocol from drifting back to an empty missing list. The preflight also refuses non-macOS `ready_official_webdriver` unless an explicit `desktop restart harness launch command` exists, so driver binaries alone cannot turn partial restart evidence into a true desktop force-quit claim. The mocked restart smoke remains the fallback rather than a true desktop force-quit claim.
...
- Remaining manual status: rows #1/#2/#3 now have archived live Forge final-answer, changed-file/diff, build/check, and confirmation-behavior evidence. The full ten-row stability batch still has partial/manual-only coverage for rows outside the disposable edit/build loop, and true Tauri force-quit/reopen remains manual-only.
...
Evidence collector: row archives now include collector output for row #1 (`2026-06-28-row-1.*`), row #2 (`2026-06-28-row-2.*`), and row #3 (`2026-06-28-row-3.*`) under `apps/desktop/docs/product/evidence/phase8-disposable-loop/`.
Evidence validator: strict row validation passed for rows #1/#2/#3 with `requireComplete: true`, `pass: true`, and empty `blockingIssues`.
Evidence archive: row #1/#2/#3 validation JSON, evidence JSON, and markdown sidecars are present. Acceptance checks these files and this log through `disposable edit/build loop beta-log archive status`.
Manual evidence template/review/finalizer: these remain the reproducible path for future rows or reruns; row #1/#2/#3 are already filled and archived.
Live row status: `node scripts/phase8-disposable-loop-status.mjs --json --skip-ui-preflight` reports `status: "complete"`, `nextRow: null`, and `liveReadyGate.reason: "complete"` from the archived row triplets. The UI-evidence preflight and doctor remain useful for future live rows, but row #1/#2/#3 no longer depend on a fresh local screenshot check.
