...
- Rows #1/#2/#3 live row runbook added on 2026-06-28: `node scripts/phase8-disposable-loop-runbook.mjs --json --row 1` reports `status: pending_live_evidence`, `readyForLiveRun: true`, the exact row #1 prompt, `/tmp/phase8-row-1-manual.json`, and the collect/validate/archive commands for `/Users/cabbos/project/forge-test-app-phase8-clean`.
- Rows #1/#2/#3 live row status helper added on 2026-06-28: `node scripts/phase8-disposable-loop-status.mjs --json` reports `status: ready_for_live_row`, `nextRow: "1"`, no archived row evidence yet, and the next row command sequence. It prevents row-order drift once strict row archives start landing.
- Rows #1/#2/#3 desktop UI observer preflight added on 2026-06-28: `node scripts/desktop-ui-evidence-preflight.mjs --json` distinguishes Forge runtime evidence from local automation visibility. Current local investigation found Tauri dev starts, Vite serves `http://localhost:1420/`, and Forge logs session creation, while System Events reported zero windows for all visible apps and Computer Use could not read Chrome either; this makes screenshots/window counts untrustworthy for live evidence in this desktop session.
...
Live row runbook: `node scripts/phase8-disposable-loop-runbook.mjs --json --row 1` currently reports the clean target is ready and prints the command sequence for row #1.
Live row status: `node scripts/phase8-disposable-loop-status.mjs --json` currently reports the next incomplete row is row #1.
