...
- Rows #1/#2/#3 live row status helper added on 2026-06-28 and connected to desktop UI evidence preflight/recovery on 2026-06-28: `node scripts/phase8-disposable-loop-status.mjs --json` currently reports `status: ui_evidence_not_ready`, `nextRow: "1"`, `readyForLiveRun: false`, no archived row evidence yet, the next row command sequence, top-level `recoveryCommands` for the desktop UI evidence doctor, opt-in settings opener, one-command recovery checks, strict preflight rerun, and live-ready hard gate, and `liveReadyGate.pass/reason` explaining whether the hard live-ready gate passed or why it is blocked. When UI preflight is skipped, the top-level recovery commands still include the strict preflight rerun and live-ready hard gate. It only treats an archive as complete when validation JSON, evidence JSON, and markdown sidecars all exist. `scripts/acceptance.sh` now also runs `node scripts/desktop-ui-evidence-doctor.mjs --json --run-checks` and `node scripts/phase8-disposable-loop-status.mjs --json --require-live-ready`, so full acceptance fails instead of silently continuing while the next row is blocked, and it intentionally fails if UI preflight is skipped. This is because the prepared project is clean but the current desktop session reports `screen_capture_limited`; it prevents row-order drift without pretending local screenshots are usable.
...
Live row runbook: `node scripts/phase8-disposable-loop-runbook.mjs --json --row 1` currently reports the clean target is ready, prints the command sequence for row #1, includes top-level recovery commands for the UI evidence doctor/settings/preflight/hard-gate path before collection/finalization, and carries `liveReadyGate` diagnostics so a single row runbook explains whether the hard gate is blocked by unchecked or unusable UI evidence.
Live row status: `node scripts/phase8-disposable-loop-status.mjs --json` currently reports `ui_evidence_not_ready` with normalized top-level recovery commands, and `--require-live-ready` can hard-fail automation while this remains blocked; the next incomplete row is still row #1, but local screenshot/window evidence must be fixed or replaced with a trusted manual desktop session before strict archive.
