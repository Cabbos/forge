...
- Rows #1/#2/#3 evidence validator added on 2026-06-27: `node scripts/validate-disposable-loop-evidence.mjs --json --project /Users/cabbos/project/forge-test-app-phase8-clean` reports `status: pending_live_evidence`, `pass: true` by default, and missing manual fields plus `live_rows_not_run`; the strict gate `--row 1 --require-complete` fails with `row1_no_changes`, `row1_build_missing`, and missing manual fields. This keeps the acceptance matrix honest while providing a hard gate after live evidence is filled.
