...
mod tests {
    ...
    #[test]
    fn snapshot_restore_preserves_a2a_parent_child_task_ids() {
        let workspace = std::env::temp_dir().join(format!(
            "forge-session-a2a-child-lineage-{}",
            uuid::Uuid::now_v7()
        ));
        std::fs::create_dir_all(&workspace).expect("workspace");
        let session = AgentSession::new(
        ...
        restored_session.restore_state(Vec::new(), None, None, None, snapshot.a2a_state);
        let restored_bus = restored_session.snapshot().a2a_state.expect("a2a state");
        let parent = restored_bus.task(&parent_id).expect("parent task");
        let projection = restored_session.a2a_projection();
