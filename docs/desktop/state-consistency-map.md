...
| Gateway trigger run evidence | Rust `TriggerRunRecord` persisted by gateway runner | JSON-line gateway protocol, IPC `GatewayTriggerRunRecord`, Settings diagnostics run rows, embedded dashboard, `forge_trigger show` | trigger runner completion/failure, `list_trigger_runs`, `runtime_status`, `dashboard_snapshot`, `get_trigger_run` | retry/dead-letter rows lose executor, failure category, or lease-expiry facts outside raw JSON | `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml gateway --lib`, `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml --bin forge_trigger render_trigger_run_detail_lines_include_runtime_evidence`, `node --test apps/desktop/src/components/settings/diagnosticsRuntimeView.test.ts --test-name-pattern "trigger run"` |
| Gateway run state | Rust `TriggerRunStore` plus gateway JSON-line status | Settings gateway runtime, CLI trigger status, acceptance restart smoke | trigger enqueue/claim/complete/retry/dead-letter, gateway restart | trigger appears lost or running after restart | `npm --prefix apps/desktop run smoke:gateway:restart -- --json --dry-run` and `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml gateway::runner --lib` |
