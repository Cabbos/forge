...
**Status:** Tasks 1-4 complete for the implemented Phase 8 slice. Follow-up acceptance gates now cover permission-policy/live-session sync and `/code-review` calibration. The disposable-project readiness preflight, clean worktree preparation, evidence collector, strict validator, archive helper, manual JSON template/reviewer, row finalizer, row runbook, and row status helper now form a repeatable edit/build evidence path. Rows #1/#2/#3 have archived 2026-06-28 validation/evidence/markdown triplets with final-answer, changed-file/diff, build/check, confirmation-behavior, transcript, and screenshot references. Desktop UI observer preflight remains wired into future row readiness so local automation window/screenshot failures report `ui_evidence_not_ready` instead of misleading live-ready state, and preflight/doctor/status/runbook outputs map that blocker to Screen Recording/Accessibility recovery commands while clarifying that Forge Trust/Full Access does not grant those macOS privacy permissions. True Tauri force-quit restart smoke remains manual/not-run evidence.
...
- **P8-A3 Disposable Loop Proof:** A disposable-project edit/build loop records exact changed files, build/check result, and final-answer evidence without manual controller writes. Current state: backend policy and acceptance-gate evidence cover rows #1/#2/#3, `scripts/disposable-loop-preflight.mjs` records whether a project is ready for fresh evidence, `scripts/prepare-disposable-loop-project.mjs` can create a clean target without resetting a dirty source project, `scripts/desktop-ui-evidence-preflight.mjs` records whether local desktop automation can be trusted for live UI evidence, including nonblank screenshot capture, a macOS privacy `permissionScope`, and normalized recovery commands, `scripts/desktop-ui-evidence-doctor.mjs` diagnoses Screen Recording/Accessibility recovery when that preflight is blocked, separates macOS privacy permissions from Forge permission modes, and points commands/actions/next-step copy back to the `--require-live-ready` hard gate, `scripts/phase8-disposable-loop-status.mjs` reports archived row coverage and now returns `complete` when row #1/#2/#3 validation/evidence/markdown triplets are present, `scripts/phase8-disposable-loop-runbook.mjs` prints row prompts and recovery commands for future reruns, `scripts/create-disposable-loop-manual-json.mjs` and `scripts/review-disposable-loop-manual-json.mjs` prevent manual field drift, `scripts/collect-disposable-loop-evidence.mjs` captures changed-file/diff/build packets, `scripts/finalize-disposable-loop-row.mjs` combines manual review, strict validation, and archive, `scripts/validate-disposable-loop-evidence.mjs` provides the strict completion gate, and `scripts/archive-disposable-loop-evidence.mjs` archives validated packets. Rows #1/#2/#3 are archived complete under `apps/desktop/docs/product/evidence/phase8-disposable-loop/2026-06-28-row-{1,2,3}.*`.
...
- Batch table: rows #1/#2/#3 now reference the archived 2026-06-28 validation/evidence/markdown triplets for final-answer, changed-file/diff, build/check, confirmation-behavior, transcript, and screenshot evidence.
- Beta log: `Phase 8 Disposable Edit/Build Loop - 2026-06-27` records `Status: Archived complete for rows #1/#2/#3` and points to `2026-06-28-row-1`, `2026-06-28-row-2`, and `2026-06-28-row-3` evidence triplets.
...
- Follow-up acceptance gate: `test: sync phase8 archive status gate` adds `disposable edit/build loop beta-log archive status` to `scripts/acceptance.sh`, requiring row #1/#2/#3 validation files and beta-log archive references to stay in sync.
...
- The ten-row stability regression batch is still not fully run end-to-end; the beta log now marks the Phase 8 disposable edit/build loop rows #1/#2/#3 as archived complete.
- Rows #7/#8 have product-level mocked UI safety evidence plus backend policy evidence.
- Rows #1/#2/#3 now have archived live disposable-project final-answer/diff/build/check/confirmation evidence under `apps/desktop/docs/product/evidence/phase8-disposable-loop/`.
...
- Restart remains manual-only until a real desktop restart harness exists. The acceptance matrix now runs `node scripts/desktop-restart-harness-preflight.mjs --json`, `node --test scripts/desktop-restart-harness-preflight.test.mjs`, and the `desktop restart harness blocker documentation status` gate so the current `blocked_official_macos` state is explicit instead of implied, including the official macOS WKWebView WebDriver support gap even when local WebDriver dependencies are later added.
