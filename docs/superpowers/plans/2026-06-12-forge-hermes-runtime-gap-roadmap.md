...
- [ ] 6.1 Implement background gateway service binary and IPC contract.
  ...
  - **Phase 6.1 attach-control follow-up (2026-06-15):** Gateway protocol now exposes `attach_session`, classifying a requested session as `live`, `restored`, `stale`, or `missing` and returning the session metadata when known. The Rust client has a typed request builder and `forge session attach <session_id>` surfaces the result. This is the control-plane step before true stream/control attachment; live agent loops still remain owned by the desktop runtime.
