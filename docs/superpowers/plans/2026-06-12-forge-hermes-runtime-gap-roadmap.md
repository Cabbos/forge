...
- [ ] 6.1 Implement background gateway service binary and IPC contract.
  ...
  - **Phase 6.1 session-input-inbox follow-up (2026-06-15):** Gateway now has a durable session input inbox (`~/.forge/session-inputs.json`) and `enqueue_session_input` RPC. `forge session input <session_id> <message>`, Tauri IPC, and TypeScript IPC can queue input addressed to an existing session, and `runtime_status`/Diagnostics expose pending input count. Desktop polling/execution is still deferred, so the attach control contract remains honest about direct send-input support.
