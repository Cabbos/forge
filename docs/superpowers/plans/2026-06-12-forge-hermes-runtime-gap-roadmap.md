...
- [x] 1.1 Extend/version the existing `AgentSessionSnapshot` schema rather than defining it from scratch.
  ...
  - *Status sync (2026-06-16): `latest_turn`, workflow/delivery projections, pending confirmations, and active tool descriptors now form the persisted resume schema. Live adapter requests are not re-associated after restart; Phase 1.6 intentionally replays interrupted tool calls as completed error blocks instead of pretending the original adapter/tool process survived.*
...
- [x] 1.3 Wire startup hydration: at startup list snapshots, call `restore_session_from_snapshot` / `emit_restored_session_startup`, and rehydrate the frontend Zustand/IndexedDB mirror. **Phase 1C (2026-06-12):** minimal app-startup session restore — `choose_startup_snapshot` selection strategy (active wins, fallback to most recent, skip if live), `startup_restore_active_session` orchestrator wired from `lib.rs` setup, 7 focused tests. **Phase 1.3 replay follow-up (2026-06-16):** frontend hydration now has browser-level coverage for an active restored session receiving the startup replay sequence (`session_started` → `resuming` → replayed `confirm_ask` → `running`) and rendering the interrupted confirmation through the persisted active-session mirror.
...
- [x] 1.5 Implement pending-confirm replay descriptors: persist interrupted confirmations so the frontend can re-render and replay them on resume.
...
- [x] 1.10 Add session store management primitives: stats, search, export, and prune for persisted snapshots.
  ...
  - **Phase 1.10 rename follow-up (2026-06-16):** Added persisted snapshot rename support by updating the saved snapshot summary through the existing load/save path. `forge_session rename <session_id> <summary>`, Tauri IPC, and TypeScript wrappers now expose the same rename primitive, closing the Hermes-style session rename gap without touching the live agent loop.
...
- [x] 3.5 Build Settings > Tools/Providers/Skills panel with tabs, search, enable toggles, and config drawers.
  ...
  - **Phase 3-G provider inventory follow-up (2026-06-16):** Added model-provider rows to the same ecosystem surface. `list_capabilities` / `list_ecosystem_items` now include `provider:*` items derived from API key status, with healthy/unavailable status, default-model config summaries, and a read-only Settings > Tools "模型" tab. Provider configuration still lives in Settings > Models; this panel exposes inventory/health rather than duplicating key editing.
  - Files: `src-tauri/src/harness/capability.rs`, `src-tauri/src/ipc/capability_handlers.rs`, `src/components/settings/SettingsCenterShell.tsx`, `CapabilityManager.tsx`, `CapabilityContentViews.tsx`, `CapabilityRows.tsx`, `CapabilityDetailDrawer.tsx` (new), `src/hooks/queries/useEcosystemItemsQuery.ts` (new), `useToolInventoryQuery.ts` (new), `queryKeys.ts`.
...
| Settings UI enhancement | ✅ Done | Status badges, detail drawer, config awareness, provider inventory tab |
| Tool inventory & counts | ✅ Done | Tool inventory IPC, summary count in UI |
| Provider inventory | ✅ Done | API-key-backed provider rows with health status and default-model summaries |
| Diagnostics integration | ✅ Done | Unhealthy counts, status enrichment; MCP config probe added in Phase 3-C |
| Tests | ✅ Done | Rust/unit coverage plus Settings ecosystem/provider mocked-IPC acceptance coverage |
| Provider inventory | ✅ Done | Provider rows come from API key status and show default-model health/config summaries |
| Extension inventory | ⏸️ Deferred | No extension source yet; represented as unavailable |
...
- [ ] 4.1 Extend `AgentSession` and `Subagent` structs to track parent session id, child ids, and lineage.
  ...
  - **Phase 4-G parent-side projection follow-up (2026-06-17):** Added derived `child_task_ids` to `AgentA2ATaskProjection`. `AgentA2ABus::projection()` now builds a parent-id to child-id index once per projection, preserves task creation order for each child list, and omits `child_task_ids` from serialized task JSON when empty. This is projection-only convenience for the Workbench; it is not a persisted parent-side child array, not a parent-session struct, and does not auto-select a parent for normal delegate calls.
  - Files: `agent/a2a/bus.rs`, `agent/a2a/projection.rs`, `agent/a2a/supervisor.rs`, `agent/session/tools.rs`, `agent/session/tools_test.rs`, `lib/protocol.ts`, `components/messages/AgentA2ATimeline.tsx`.
...
- [ ] 4.6 Build Subagent Workbench view: tree of parent/child sessions, status badges, cost tab, file IO tab.
  ...
  - **Phase 4-G lineage UI follow-up (2026-06-17):** Added a child-count lineage indicator in the timeline/workbench when a task projection carries derived `child_task_ids`, with an accessible `aria-label` listing the child ids. This remains a projection-level indicator, not a full session tree.
...
- [ ] 4.8 Persist subagent lineage to session snapshot for resume.
  ...
  - **Phase 4-G projection follow-up (2026-06-17):** Parent-side `child_task_ids` are derived from persisted child-to-parent pointers during projection, so they are reproducible after snapshot restore without adding a persisted parent-side child-id array.
...
**Phase 4-G summary (2026-06-17):** Parent-side A2A lineage projection.

| Item | Status | Notes |
|------|--------|-------|
| Derived child ids | ✅ Done | `AgentA2ABus::projection()` precomputes `parent_task_id -> Vec<child_task_id>` once, preserves child creation order, and looks up each task's `child_task_ids` without scanning all tasks per row |
| Serialization contract | ✅ Done | Empty `child_task_ids` lists still serialize as omitted fields; populated parent projections include ordered child ids |
| Workbench indicator | ✅ Done | Timeline/workbench task rows show a child-count lineage badge with an accessible label listing child ids |
| Product smoke mocks | ✅ Done | A2A e2e mocks include `child_task_ids`; parent mock tasks include two child ids and assertions cover the child-count indicator |
| Not claimed | 🚫 Deferred | `child_task_ids` is projection-only; no persisted parent-side child-id array, no parent-session struct, and no automatic parent selection for ordinary delegates |

Phase 4-G evidence:

```bash
cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml agent::a2a::bus::tests::projection_derives_parent_child_task_ids_in_creation_order --lib # pass, 1 passed
npm --prefix apps/desktop run test:e2e -- e2e/a2a-confirm-runtime.spec.ts -g "hub panel renders detailed" # pass, 1 passed
...
- [x] 5.3 Add Settings > Memory panel: view, search, edit, delete facts.
  ...
  - **Phase 5-G acceptance follow-up (2026-06-16):** The final Settings smoke now exercises active-profile switching and the Memory panel's scoped fact list. It verifies that work-profile facts are visible, other-profile facts stay hidden, search respects the active profile, and newly created facts send `profile_id` in the IPC payload.
- [x] 5.10 (partial) Tests: memory store unit tests.
  ...
  - **Phase 5-G acceptance follow-up (2026-06-16):** `e2e/acceptance.spec.ts` now covers the user-facing memory/profile scope contract through mocked Tauri IPC rather than only pure helper/backend tests.
...
- [x] 5.5 Add profile switcher in UI and CLI `--profile` flag.
  ...
  - **Phase 5-H acceptance follow-up (2026-06-16):** The final Settings smoke now covers the profile CRUD surface directly: create a profile with provider/model/workspace defaults, edit those defaults, and delete the non-active profile while asserting the Tauri IPC payloads.
...
- [x] 6.3 Add macOS `launchd` plist generation and registration.
  - **Phase 6.3 verification (2026-06-16):** `service/launchd.rs` now owns plist generation (`generate_plist`), LaunchAgents path/log paths, install/bootstrap, uninstall/bootout, start/stop/restart/status, and CI-safe runner seams. `forge_service` routes lifecycle commands through the service facade, and diagnostics/autostart consume the same structured service status.
  - Files: `service/launchd.rs`, `service/mod.rs`, `src-tauri/src/bin/forge_service.rs`.
- [x] 6.4 Add Windows service wrapper and Linux systemd unit generation.
...
- [x] 6.5 Add autostart toggle in Settings > General.
  ...
  - **Phase 6.5 acceptance follow-up (2026-06-16):** Added mocked acceptance coverage for Settings > General autostart enable/disable, verifying `set_autostart` payloads and UI refresh from the returned cross-platform service status.
  - Files: `src/components/settings/GeneralSettings.tsx`, `e2e/acceptance.spec.ts`, `e2e/fixtures/app.ts`.
- [x] 6.6 Add dashboard: lightweight web UI served by gateway showing sessions, health, and logs.
  ...
  - **Phase 6.6 dashboard surface follow-up (2026-06-16):** The gateway root dashboard now renders the existing snapshot into read-only Runtime, Runtime Tasks, Sessions, Queued Triggers, Recent Runs, Session Inputs, and Event Log sections instead of exposing only a raw JSON dump. The page remains dependency-free static HTML/JS served by the gateway and continues to consume `/api/dashboard`.
  - Files: `src-tauri/src/gateway/dashboard.rs`, `src-tauri/src/gateway/server.rs`, `src-tauri/src/bin/gateway.rs`.
- [x] 6.7 Implement update repair: on update, detect stale gateway/config, run doctor, repair if needed.
  - **Phase 6.7 planner follow-up (2026-06-15):** Added `diagnostics/update_repair.rs` as the update-repair planning/execution layer. It converts `DiagnosticsReport` warnings/failures into deduplicated repair actions, keeps unrepairable failures as manual blockers, and executes the plan through an injectable runner (real path uses `run_repair`). Follow-up safety hardening restricts automatic update repair to conservative service lifecycle actions (`restart_gateway`, `reinstall_service`); destructive or unknown actions such as clearing snapshots, A2A ledgers, or logs are retained as manual blockers. Focused tests cover gateway repair planning, service-action allowlisting, destructive/unknown action blocking, manual blockers for config failures, action deduplication, ordered execution, and failed repair results.
  - **Phase 6.7 lifecycle follow-up (2026-06-16):** App startup now tracks update repair once per app version using a marker at `~/.forge/update-repair-state.json`. A missing marker creates a baseline without running repairs, so fresh installs do not silently enable autostart; when an existing marker's version differs, Forge runs basic diagnostics and executes only the conservative update-repair allowlist (`restart_gateway`, `reinstall_service`) in a background thread, logs the outcome, and records the marker so failed/manual-blocked checks do not repeatedly fire on every launch.
  - Files: `diagnostics/update_repair.rs`, `diagnostics/mod.rs`, `lib.rs`.
- [x] 6.8 Add self-healing actions from diagnostics: restart gateway, clear snapshot cache, reinstall service.
  - **Phase 6.8 verification follow-up (2026-06-15):** `RepairResult` now carries optional post-action verification detail. `restart_gateway` and `reinstall_service` verify platform service status after the repair command and fail honestly when the service is still not running, rather than reporting command success as repair success. Settings > Diagnostics formats verification detail in the repair result message.
  ...
  - **Phase 6.8 repair-service follow-up (2026-06-16):** Gateway repair actions now restart, reinstall, and verify the background service through the cross-platform service facade instead of calling launchd directly. The verification path accepts launchd, systemd, and Windows Service running messages, and `diagnostics/repair.rs` no longer imports platform-specific service modules.
...
- [x] 7.3 Implement recovery/error states: offline, gateway disconnected, API key missing, snapshot corrupted.
  ...
  - **Phase 7.3 offline follow-up (2026-06-16):** AppShell now renders a global offline banner when the browser reports `navigator.onLine === false` or receives an `offline` event, and removes it on `online`. Playwright covers the offline/online transition alongside recovery and health alert surfaces.
  - Files: `src/components/layout/RecoveryNoticeBanner.tsx`, `src/components/layout/HealthAlertBanner.tsx`, `src/components/layout/NetworkStatusBanner.tsx`, `store/index.ts`.
...
- [x] 7.5 Implement rich previews: image diff, file tree diff, markdown preview for writes.
  ...
  - **Phase 7.5 image-write-preview follow-up (2026-06-16):** Image writes now render inside expanded tool details when the payload contains an inline SVG, image data URL, or base64 raster content for common image extensions. Added pure derivation coverage and Playwright coverage that asserts the rendered image has a real layout box.
  - **Phase 7.5 image-diff follow-up (2026-06-16):** Expanded diff cards now derive before/after image previews from `diff_view` old/new content for common image extensions, using the existing stream event contract. Playwright covers rendered SVG before/after previews with non-zero layout boxes. Pixel-delta heatmaps remain out of scope.
  - Files: `src/components/messages/WriteFilePreview.tsx`, `src/components/messages/DiffCard.tsx`, `src/components/messages/diffPresentation.ts`.
- [x] 7.6 Harden review flows: approve/reject UI, bulk review, review history.
  ...
  - **Phase 7.6 review-actions follow-up (2026-06-16):** Added a durable `review_agent_a2a_tasks` IPC contract backed by the A2A bus and ledger/snapshot persistence. Agent Workbench now exposes single-item and bulk approve/reject actions, writes the returned projection into the active store, and records approved/rejected decisions in review history. Rust, helper, and Playwright coverage validate approve/reject state transitions and the Hub approval path.
  - Files: `src-tauri/src/agent/a2a/bus.rs`, `src-tauri/src/ipc/a2a_handlers.rs`, `src/components/messages/AgentA2ATimeline.tsx`, `src/lib/workbenchSummary.ts`, `e2e/a2a-confirm-runtime.spec.ts`.
...
- [x] 7.8 Final acceptance suite: end-to-end script covering resume, doctor, tool enable/disable, subagent, scheduler, settings round-trip.
  ...
  - **Phase 7.8 scheduler-trigger contract follow-up (2026-06-16):** Acceptance scheduler mocks now match the backend run-now contract: pressing "立即运行" records a `queued` history entry and appends a pending gateway trigger with the task prompt, profile, and workspace metadata instead of simulating an already-completed local run.
  - **Phase 7.8 memory-profile gate follow-up (2026-06-16):** Acceptance mocks now include profile and memory-fact IPC state so the smoke suite verifies active-profile memory isolation and profile-scoped fact creation from Settings.
  - **Phase 7.8 profile-crud gate follow-up (2026-06-16):** Acceptance now verifies Settings > Profiles create/edit/delete round trips through the same mocked IPC contract used by the app shell.
...
- ✅ Phase 6.7: Update repair planner/runner maps diagnostics to repair actions, allows only conservative service repair actions to run automatically, keeps destructive/unknown actions as manual blockers, and runs once per app version from startup
- ✅ Phase 6.8: Gateway repair actions now verify post-repair service health and surface verification detail in Diagnostics
